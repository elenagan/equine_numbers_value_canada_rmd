...
```{r plot_horses, fig.width=10, fig.height=3, fig.cap="Fig.1: Historical Number of Horses in Canada by province"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1: Maximum and Minumum Number of Horses per province")
```

As we can see above, Saskatchewan has the highest maximum number of horses at `r max_horses[[2]][9]` and Prince Edward Island has the lowest minimum number of horses at `r max_horses[[3]][7]`.

Below we zoom in and look at the province of `r params$province`:

```{r plot_province, fig.width=4, fig.height=4, fig.cap="Fig.2: Historical Number of Horses in a specific province"}
